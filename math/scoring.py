...
def full_house_points(dice_vec):
    return 25 * (((3 in dice_vec) and (2 in dice_vec)) or (5 in dice_vec))

def small_straight_points(dice_vec):
    return 30 * (np.prod(dice_vec[0:4]) + np.prod(dice_vec[1:5]) + np.prod(dice_vec[2:6]) > 0)

def large_straight_points(dice_vec):
    return 40 * (np.prod(dice_vec[0:5]) + np.prod(dice_vec[1:6]) > 0)

def chance_points(dice_vec):
    return get_sum(dice_vec)

def yahtzee_points(dice_vec):
    return 50 * (5 in dice_vec)
...
def joker_points(dice_vec, scoring_function_index):
    if scoring_function_index == 9:
        return 30
    elif scoring_function_index == 10:
        return 40
    else:
